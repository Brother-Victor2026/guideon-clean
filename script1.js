...
async function switchSession(id){
  ...
  try{
    ...
    const data=await r.json();
    if(Array.isArray(data)&&data.length>0)data.forEach(m=>am(m.role==='assistant'?'guideon':m.role,m.content));
    else{
      const hr=new Date().getHours();
      const moment=hr<12?'matin':hr<18?'après-midi':'soir';
      const salut=hr<12?'Bonjour':hr<18?'Bon après-midi':'Bonsoir';
...
window.onerror=(m,u,l,c,e)=>alert('ERR:'+m+'@'+l+':'+c);window.addEventListener('unhandledrejection',e=>alert('REJ:'+e.reason));